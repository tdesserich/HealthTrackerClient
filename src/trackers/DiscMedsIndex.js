...
class DiscMedsIndex extends Component {
    ...
    discMedsUpdate = (event, updatedMeds) => { 
        fetch(`https://tdesserich-healthtrackerserver.herokuapp.com/discmeds/${updatedMeds.id}`, {
            method: 'PUT', 
    ...
    discMedsDelete = (event, deletedMeds) => { 
        fetch(`https://tdesserich-healthtrackerserver.herokuapp.com/discmeds/${deletedMeds.id}`, {
            method: 'DELETE', 
            headers: new Headers({
                'Content-Type': 'application/json',
                'Authorization': this.props.token
    ...
    render() {
        const discMeds = this.state.discMeds.length >= 1 ?
            <DiscMedsDisplay discMeds={this.state.discMeds}
                delete={this.discMedsDelete} update={this.setUpdatedMeds} /> : 
                <div className="display">
                    <h2 style={{borderBottom: "1px solid grey"}}>Log a discontinued medication to see results</h2>
                </div>    
        return (
